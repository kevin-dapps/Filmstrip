...
public class InstagramApp {
	...
	public static String mCallbackUrl = "http://instagram.com/developer/clients/manage/";
	...
	public InstagramApp(Context context, String clientId, String clientSecret,
		...
		mAuthUrl = AUTH_URL + "?client_id=" + clientId + "&redirect_uri="
				+ mCallbackUrl + "&response_type=code&display=touch&scope=basic+likes+comments+relationships";
		...
		new Thread() {
			...
			public void run() {
				...
				try {
					...
					mSession.storeAccessToken(mAccessToken, id, user, name, profile_pic,media,follows,followed_by);
				...
				mHandler.sendMessage(mHandler.obtainMessage(what, 1, 5));
			}
		}.start();
	}
		...
		new Thread() {
			...
			public void run() {
                ...
                try {
                    ...
                    String profile_pic = jsonObj.getJSONObject("data").getString("profile_picture");
                    int media = jsonObj.getJSONObject("data").getJSONObject("counts").getInt("media");
                    int follows = jsonObj.getJSONObject("data").getJSONObject("counts").getInt("follows");
                    int followed_by = jsonObj.getJSONObject("data").getJSONObject("counts").getInt("followed_by");
                    
                    
                    mSession.updateInfo(user, name, profile_pic,media,follows,followed_by);
                    // String bio =
	...
	public int getMediaNumber() {
		return mSession.getMediaNumber();
	}
