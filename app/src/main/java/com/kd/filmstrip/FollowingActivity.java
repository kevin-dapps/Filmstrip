...
public class FollowingActivity extends BaseActivity {
			...
			public void onScroll(AbsListView view, int firstVisibleItem,
	           ...
	           if(lastItem == totalItemCount) {
	              if(preLast!=lastItem){ //to avoid multiple calls for last item
	                preLast = lastItem;
	                if(next_url != null)
	                new LoadMore(next_url, FollowingActivity.this).execute();
	              }
	           }
    ...
    public static class FetchUsers extends AsyncTask<Void, String, Boolean> {
        ...
        protected Boolean doInBackground(Void... voids) {
            try {
                URL url = new URL("https://api.instagram.com/v1/users/"+userListID+"/follows?access_token=" + HomeActivity.mApp.mAccessToken);
